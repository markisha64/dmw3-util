    ...
    (
        ...
        impl $Codepoint {
            ...
            pub fn $decode(bytes: &[u8]) -> Option<(Self, &[u8])> {
                $(
                    if let Some(rest) = bytes.strip_prefix::<[u8]>(&[ $($byte as u8),* ]) {
            ...
            pub fn $encode<W: io::Write>(self, mut writer: W) -> Result<(), anyhow::Error> {
                match self {
                    $(
...
decl_codepoints! {
    Codepoint, Other, decode, encode, len, parse;
    0x00 => Null, "Null", "\\0";
    ...
    0x01, 0x49 => LatinCapitalLetterUWithDiaeresis,     "Latin capital letter u with diaeresis",        "Ü";
