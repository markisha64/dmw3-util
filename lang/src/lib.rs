...
impl LangFile {
    /// Parses a lang file.
    pub fn parse<R: io::Read + io::Seek + io::BufRead>(mut reader: R) -> Result<Self, anyhow::Error> {
    ...
    pub fn encode<W: io::Write + io::Seek>(&self, mut writer: W) -> Result<u64, anyhow::Error> {
        ...
        let strings_len =
            u32::try_from(self.strings.len()).context("Unable to convert strings' length to `u32`")?;
        writer
            .write_u32::<LittleEndian>(strings_len)
            .context("Unable to write header length")?;
        for string_pos in strings_pos {
            let string_pos =
                u32::try_from(string_pos).context("String position didn't fit into a `u32`")?;
            writer
